...
- **Influence.** Spend it on **policy fights** — a rotating board of 4 from a 50-card deck in eight classes (`FIGHT_TYPES` in `data.js`): BILL / EO / NOM are the standard tug-of-war; **HEARING** is fast, pays clout, boosts testimony (+20%, flubs are public) and can put a named rival in the hot seat (probe wins dent their donor confidence; they defend themselves); **RULE** runs a long comment period with a ✦30-per-institution monthly docket cap, so patience beats dumps; **COURT** is long and swingy (flatter odds curve) and seats a permanent ally on the bench; **APPROPS** riders carry marquee cash, every rival treats them as a pet issue, and losers get 40% of their stake back; **STATE** fights are cheap, quick, and ignored by each big shop 60% of the time (`stateRivalSkip`). Each fight is a tug-of-war between two sides; rival think tanks pile on every month — and surge hard into a fight's final month, so a last-minute influence dump meets a wall of closing money. The ⚑ attribution line under each side shows who's backing it — as the **Bugle's estimates** (`fogPct` 25%, a third of that with a Gov Relations Lead; exact after the vote or a leak), so credit-sniping is no longer arithmetic. When the clock hits zero **the wire decides**: influence shares set win odds on a contest curve (`contestK` 2.6, bent by each fight's hidden volatility `volMin`–`volMax`, so the card shows an odds *range*; a 2:1 lead wins ~80–90%, upsets happen, and the Bugle animates every roll). **Nothing is locked in:** each month a live fight may take **floor action** (`FLOOR_ACTIONS`, `floorChance` 15%, rolled a month ahead so an intel desk can warn you) — swing votes, poison pills, riders, stalled or fast-tracked clocks, motions to table, scandals, leaks — and in its final month **dark money** (`darkChance` 25%, `DARK_GROUPS`) may drop a pile on either side; if the outside group is the biggest backer at the vote, nobody banks the victory. Cash and clout (✦) rewards scale by your share of the winning side, and some fights pay in kind — a grateful scholar joining free, a warm donor intro at half court cost, or amnesty on donor strikes. Rivals sit out fights beyond their pet issues (fewer, deeper piles) and surge into final months.
...
| shrewd (builds positions, buys odds, stewards a lean base) | **~25%** | the per-tank ladder is the point now: Easy tanks 49–50%, Medium 21–26%, Hard/Expert (pooled credit, leader denial) ~0% for the bot — deliberately below the old 45–50% because a good human beats this bot handily |
